...
public class RenderBatch implements Comparable<RenderBatch>{
  // Vertex
  // ======
  // Pos            Color                 texCords       texID
  // float, float   float, float, float   float, float   float

  private final int VERTEX_PER_QUAD = 4;
  ...
  private final int VERTEX_SIZE = POS_SIZE + COLOR_SIZE + TEX_CORDS_SIZE + TEX_ID_SIZE;
  ...
  public RenderBatch(int maxBatchSize, int zIndex) {
    ...
    // 4 verticies quads
    vertices = new float[maxBatchSize * VERTEX_PER_QUAD * VERTEX_SIZE];
  ...
  public void start() {
    ...
    // Create and upload the Indicies buffer
  ...
  public void addSprite(SpriteRenderer spr) {
    ...
    // Add propertities to local verticies array
  ...
  private void loadVertexProperties(int index) {
    ...
    // Add verticies with appropriate propertities
    float xAdd = 1.0f;
    float yAdd = 1.0f;
    for (int i = 0; i < 4; i++) {
      if (i == 1) {
